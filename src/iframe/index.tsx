...
import { PrivKeyResolver, UIKeyStore } from '../keystore/UIKeyStore.js';

/**
 * To use the Aztec wallet, a dApp embeds a headless iframe that does necessary processing securely.
 * When a user UI flow is needed, the iframe creates a pop-up
 */
...
async function main() {
  ...
  const keyStore = new UIKeyStore(wasm, []);
  const server = createIframeAztecWalletProviderServer(wasm, keyStore);
  server.run();
